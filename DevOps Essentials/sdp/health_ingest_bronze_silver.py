...
# create the actual table using functions
def health_bronze():
    return (
        spark
        .readStream
        ...
        .option("header","true")
        .schema(helper_func.get_health_csv_schema())   ## Use the custom schema we created
...
def health_silver():
    return (
        ...
        .withColumn("HighCholest_Group", helper_func.high_cholest_map("HighCholest"))  # UDF - highcholest_map 
        .withColumn("Age_Group", helper_func.group_ages_map("Age"))                   # UDF - group_ages_map Age
