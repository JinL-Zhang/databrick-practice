...
def test_high_cholest_column_valid_map(spark):
    ...
    actual_df = sample_df.withColumn("actual", helper_func.high_cholest_map("value")) ## add the module prior to the function
...
def test_age_group_column_valid_map(spark):
    ...
    # Apply the transformation on the sample data
    actual_df = sample_df.withColumn("actual", helper_func.group_ages_map("value"))  ## add the module prior to the function
